...
" size="25" id="kw" class="inp" onfocus="if(this.value=='输入关键词')this.value='';"/>
<div style="padding:10px 0 0 0;">
<select name="file">
<?php if(is_array($MENU)) { foreach($MENU as $k => $v) { ?>
<?php if(in_array($menu_file[$k], array('sell', 'buy', 'news', 'credit', 'job', 'photo', 'info', 'brand', 'video', 'mall'))) { ?><option value="<?php echo $menu_file[$k];?>"<?php if($file==$menu_file[$k]) { ?> selected<?php } ?>
